    ...
    @section('content')
        <div class="container">
            ...
            <form action="{{ route('admin.stock.update', ['material' => $material->id]) }}" method="POST">
                @csrf
                <h1>Quantidade atual de {{ $material->nome }}:
                    <label>{{ $material->quantidade }}</label>
                </h1>
                <div class="quantity-form">
                    <div>
                        <input type="radio" name="ratio" id="adicionar" value="adicionar" class="adicionar">
                        <label for="adicionar" class="radio-label">Adicionar</label>
                    </div>
                    <div class="ratio">
                        <input type="radio" name="ratio" id="remover" value="remover" class="remover">
                        <label for="remover" class="radio-label">Remover</label>
                    </div>
                ...
                @if($errors->has('quantidade'))
                    <p class="message error-message">{{ $errors->first('quantidade') }}</p>
                @endif

                @if($errors->has('ratio'))
                    <p class="message error-message">{{ $errors->first('ratio') }}</p>
                @endif
